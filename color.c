...
t_get_color_func	get_next_color_func(void)
{
	static t_get_color_func	color_funcs[] = {get_color, get_color_black_white,
		get_color_red_blue, get_color_rainbow};
	static int				i = 0;
...
int	get_color(int iter, const int max_iter)
{
	if (iter == max_iter)
		return (0x000000);
	return ((int)(iter * 255.0 / max_iter) << 16 | (int)(iter * 255.0
			/ max_iter) << 8 | (int)(iter * 255.0 / max_iter));
}
...
int	get_color_rainbow(int iter, const int max_iter)
{
	...
	{
		return ((int)(255 - (iter - max_iter / 3) * 255.0 / (max_iter
				/ 3)) << 16 | 0x0000FF);
	}
	else
	{
		return ((int)((iter - 2 * max_iter / 3) * 255.0 / (max_iter
				/ 3)) << 8 | 0xFF0000);
	}
}
...
int	get_color_red_blue(int iter, const int max_iter)
{
	...
	{
		return ((int)(255 - (iter - max_iter / 2) * 255.0 / (max_iter
				/ 2)) << 8);
	}
}
